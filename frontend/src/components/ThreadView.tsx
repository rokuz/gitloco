      ...
      {!isResolved && (
        <div className="border-t border-zinc-800 p-2 space-y-2">
          <textarea
            ...
            placeholder="Reply…"
            rows={2}
            className="w-full resize-y rounded border border-zinc-700 bg-zinc-950 px-2 py-1 text-zinc-100 text-sm focus:outline-none focus:border-zinc-500"
          />
          {replyMutation.error && (
            <div className="text-xs text-red-400">{String(replyMutation.error)}</div>
          )}
...
function ReplyRow({ reply }: { reply: Reply }) {
  return (
    <li className="px-3 py-2">
      ...
      <p className="whitespace-pre-wrap text-zinc-100">{reply.body}</p>
    </li>
  );
}
